...
void adams2(double t0, double x0[], double h, int n, FILE *out) {
    ...
    for (int step = 1; step < n; step++) {
        t[step+1] = t[step] + h;
        for (int i = 0; i < 3; i++) {
            x[step+1][i] = x[step][i] + h/2 * (3*dx[step][i] - dx[step-1][i]);
        }
...
void adams3(double t0, double x0[], double h, int n, FILE *out) {
    ...
    for (int step = 2; step < n; step++) {
        t[step+1] = t[step] + h;
        for (int i = 0; i < 3; i++) {
            x[step+1][i] = x[step][i] + h/12 * (23*dx[step][i] - 16*dx[step-1][i] + 5*dx[step-2][i]);
        }
        derivatives(t[step+1], x[step+1], dx[step+1]);
        fprintf(out, "%f %f %f %f\n", t[step+1], x[step+1][0], x[step+1][1], x[step+1][2]);
    }
...
void adams4(double t0, double x0[], double h, int n, FILE *out) {
    ...
    for (int step = 3; step < n; step++) {
        t[step+1] = t[step] + h;
        for (int i = 0; i < 3; i++) {
            x[step+1][i] = x[step][i] + h/24 * (55*dx[step][i] - 59*dx[step-1][i] + 37*dx[step-2][i] - 9*dx[step-3][i]);
        }
        derivatives(t[step+1], x[step+1], dx[step+1]);
        fprintf(out, "%f %f %f %f\n", t[step+1], x[step+1][0], x[step+1][1], x[step+1][2]);
    }
...
int main() {
    ...
    char filename[20];
    sprintf(filename, "adams%d.dat", ADAMS_ORDER);
    FILE *out = fopen(filename, "w");
    
    switch(ADAMS_ORDER) {
        case 2: adams2(t0, x0, h, n, out); break;
        case 3: adams3(t0, x0, h, n, out); break;
        case 4: adams4(t0, x0, h, n, out); break;
        default: fprintf(stderr, "Unsupported Adams order\n"); return 1;
    }
